...
namespace Personal_takip_1
{
    public partial class frmSifremiUnuttum : Form
    {
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            string soru = comboSoru.Text;
            string cevap = txtSifreTekrar.Text;

            if (KullaniciSoruCevapEslesiyorMu(k.KullaniciID, soru, cevap))
            {
                // Şifre güncelleme işlemini gerçekleştir
            }
            else
            {
                MessageBox.Show("Şifre hatırlatma sorusu ve cevabı eşleşmiyor!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private bool KullaniciSoruCevapEslesiyorMu(int kullaniciID, string soru, string cevap)
        {
            
            string sql = "SELECT COUNT(*) FROM Kullanicilar WHERE KullaniciID = @KullaniciID AND Soru = @SifreHatirlatmaSorusu AND Cevap = @Cevap";
        ...
        private void comboSoru_SelectedIndexChanged(object sender, EventArgs e)
        {
